...
////////////////////////////////////////////////////////
 ...
 // Description: Helps with php debugging
	...
	static function d(&$var, $info = FALSE)
	{
		$scope = false;
		$prefix = 'unique';
		$suffix = 'value';
		...
		echo "<pre style='margin: 0px 0px 10px 0px; display: block; background: white; color: black; font-family: Verdana; border: 2px solid #C54BD8; padding: 5px; font-size: 10px; line-height: 13px;white-space : nowrap '>";
	...
	static function dh(&$var, $var_name = NULL, $indent = NULL, $reference = NULL)
	{
		...
		else
		{
			...
			if(is_array($avar))
			{
				...
				{
					$value = &$avar[$name];
					self::dh($value, "['$name']", $indent.$do_dump_indent, $reference);
				}
				echo "$indent)<br>";
			}
		 	elseif(is_object($avar))
		 	{
				...
				foreach($avar as $name=>$value) self::dh($value, "$name", $indent.$do_dump_indent, $reference);
				echo "$indent)<br>";
			}
