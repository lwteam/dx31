...
if (CURMODULE == 'index') {
	...
	header('Location: http://bbs.lenovo.com/');
	die();
	...
	if ($id) {
		$location = "http://bbs.lenovo.com/forum.php?mod=forumdisplay&fid=$id&page=$page";
	}else{
		$location = "http://bbs.lenovo.com/";
	}
	...
	$id = DB::result_first("SELECT tid FROM ".DB::table('forum_thread_lephonetid')." WHERE lephonetid='$id'");
	if ($id) {
		$fid = DB::result_first("SELECT  tid FROM ".DB::table('forum_thread_lephonetid')." WHERE lephonetid='{$thread['fid']}'");
		$location = "http://bbs.lenovo.com/forum.php?mod=viewthread&fid=$fid&tid=$id&page=$page";
	}else{
		$location = "http://bbs.lenovo.com";
	}
...
/* ModuleSystem Point */
if(ModulePointWhether('hooks',__FUNCTION__,__CLASS__)){
	UNSET($GLOBALS['_ModuleActionStorage' ]);
	$Modextract = ModulePoint('hooks',__FUNCTION__,__CLASS__);
