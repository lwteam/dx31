...
function dheader($string, $replace = true, $http_response_code = 0) {
	...
	if(ModulePointWhether('output',__FUNCTION__,__CLASS__)){
		...
		$returns['http_response_code'] = $http_response_code;
...
function template($file, $templateid = 0, $tpldir = '', $gettplfile = 0, $primaltpl='') {
	...
	/* ModuleSystem Point */
	if(ModulePointWhether('template') && !defined('DIS_ModuleSystem')){
		$GLOBALS['_ModuleActionStorage' ] = $file;
		$Modextract = ModulePoint('template');
		if(isset($Modextract) && is_array($Modextract)){
			extract($Modextract);
			unset($Modextract);
		}
...
function output() {
	...
	/* ModuleSystem Point */
	if(ModulePointWhether('output')){
		UNSET($GLOBALS['_ModuleActionStorage' ]);
		$Modextract = ModulePoint('output');
		if(isset($Modextract) && is_array($Modextract)){
			extract($Modextract);
			unset($Modextract);
		}
...
function showmessage($message, $url_forward = '', $values = array(), $extraparam = array(), $custom = 0) {
	...
	if(ModulePointWhether('output',__FUNCTION__,__CLASS__)){
		...
		$returns['url_forward'] = $url_forward;
		$returns['values'] = $values;
		$returns['extraparam'] = $extraparam;
