...
function debugmessage($ajax = 0) {
	$m = function_exists('memory_get_usage') ? sprintf('%01.2f', memory_get_usage()/1024/1024) : '';
	$mt = function_exists('memory_get_peak_usage') ? sprintf('%01.2f', memory_get_peak_usage()/1024/1024) : '';
	if($m) {
		$m = '<em>内存:</em> <s>'.$m.'</s> MB'.($mt ? ', 峰值 <s>'.$mt.'</s> MB' : '').'<br />';
	}
	global $_G;
	$debugfile = TRUE ? '_debugadmin.php' : '_debug.php';
	$akey = md5(TIME);
...
EOF;
	...
	$debug .= '
		...
		(TRUE ? '<img src="../static/image/common/arw_r.gif" /><a id="__debug_6" href="#debugbar" onclick="switchTab(\'__debug\', 6, 6)">$_G</a>' : '').
		(TRUE ?
