...
0
|| checktplrefresh('./template/default/forum/viewthread.htm', './template/default/forum/viewthread_node.htm', 1396851631, 'diy', './data/template/2_diy_forum_viewthread.tpl.php', './template/new', 'forum/viewthread')
|| checktplrefresh('./template/default/forum/viewthread.htm', './template/default/forum/viewthread_fastpost.htm', 1396851631, 'diy', './data/template/2_diy_forum_viewthread.tpl.php', './template/new', 'forum/viewthread')
|| checktplrefresh('./template/default/forum/viewthread.htm', './template/new/forum/viewthread_node_body.htm', 1396851631, 'diy', './data/template/2_diy_forum_viewthread.tpl.php', './template/new', 'forum/viewthread')
|| checktplrefresh('./template/default/forum/viewthread.htm', './template/new/common/seditor.htm', 1396851631, 'diy', './data/template/2_diy_forum_viewthread.tpl.php', './template/new', 'forum/viewthread')
|| checktplrefresh('./template/default/forum/viewthread.htm', './template/default/forum/seccheck_post.htm', 1396851631, 'diy', './data/template/2_diy_forum_viewthread.tpl.php', './template/new', 'forum/viewthread')
|| checktplrefresh('./template/default/forum/viewthread.htm', './template/new/common/upload.htm', 1396851631, 'diy', './data/template/2_diy_forum_viewthread.tpl.php', './template/new', 'forum/viewthread')
|| checktplrefresh('./template/default/forum/viewthread.htm', './template/new/common/seccheck.htm', 1396851631, 'diy', './data/template/2_diy_forum_viewthread.tpl.php', './template/new', 'forum/viewthread')
...
<?php } if(BUGLIST) { ?>
<style>
.buglistop{padding:10px;border-left: 1px solid #CDCDCD;background: #F2F2F2;margin-bottom:10px}
.select{font-weight:700}
</style>
<div class="buglistop"><?php if(is_array($_Data['buglist_handling'])) foreach($_Data['buglist_handling'] as $key => $value) { if($bthread['handlingnum']==1 && $bthread['handling']==4 && !in_array($key,array('0','4'))) { continue;?><?php } else { } if($key<4 || $bthread['handlingnum']==1 && $bthread['handling']==4 ) { ?>
<span<?php if($bthread['handling'] == $key) { ?> class="select"<?php } ?>><?php echo $value['title'];?> -></span>
<?php } } ?>
</div>
<?php if($bthread['hardware'] ||$bthread['version'] ) { ?>
<div class="buglistop">
<?php if($bthread['hardware']) { ?>机型: <?php echo $BugOption['hardware'][$bthread['hardware']]['title'];?><?php } ?>
  <?php if($bthread['version']) { ?>版本:<?php echo $BugOption['version'][$bthread['version']]['title'];?><?php } ?>
</div>
<?php } if($bthread['supply']) { ?>
<div class="buglistop">
<?php if($bthread['supplytime']) { ?>
<span title="用户已经于<?php echo date('Y-m-d H:i',$bthread['supplytime']);; ?>补充信息" style="color:#EE00EE">[已经补充信息]</span>
<?php } else { ?>
<span title="工程师已设置该问题需要用户补充信息" style="color:#EE82EE">[需要补充信息]</span>
<a  onclick="BMDialog.get(this,<?php echo $bthread['tid'];?>,null,true);" href="javascript:void(0);" title="处理该问题的流程">补充</a>
<?php } ?>
</div>
<?php } if(1) { ?>
<div class="buglistop">
<span>管理:</span>
<a onclick="BMDialog.get(this,<?php echo $bthread['tid'];?>);" href="javascript:void(0);" title="处理该问题的流程">处理这个问题</a>
<a onclick="BMDialog.property(<?php echo $bthread['tid'];?>,'#show_<?php echo $bthread['tid'];?>');" href="javascript:void(0);" >查看属性</a>
<div id="show_<?php echo $bthread['tid'];?>" style="display:none"></div>
</div>

<?php } if($bthread['hlog']) { ?>
<div class="buglistop">
<span>流程日志:</span><?php if(is_array($bthread['hlog'])) foreach($bthread['hlog'] as $value) { ?><p><?php echo date('m-d H:i',$value['dateline']);; ?>  

<?php if($value['dist']) { if($value['hide']) { ?><?php echo $value['another'];?><?php } else { ?><?php echo $value['name'];?><?php } } else { ?><?php echo $value['username'];?>
<?php } ?>

<?php echo $value['message'];?>


<?php if(Library::getstatus($value['status'],1)) { ?>
[工程师设置需要用户补充]
<?php if(Library::getstatus($value['status'],2)) { ?>[用户补充]<?php } } else { ?>
状态更改为 <span style="color:<?php echo $_Data['buglist_handling'][$value['handling']]['color'];?>"><?php echo $_Data['buglist_handling'][$value['handling']]['title'];?> </span>
<?php } ?>
...
<div class="buglistop">
<div><span id="samenum"><?php echo $bthread['samenum'];?></span>个人也遇到过这个问题</div>
<a onclick="BMDialog.samenum('<?php echo $bthread['tid'];?>','#samenum');" href="javascript:void(0);" title="处理该问题的流程">我也遇到过</a>
</div>
